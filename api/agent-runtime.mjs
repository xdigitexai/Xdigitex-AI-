...
`ALTER TABLE agent_tool_calls ADD COLUMN IF NOT EXISTS task_item_id BIGINT REFERENCES agent_task_items(id) ON DELETE SET NULL`,
`ALTER TABLE agent_tool_calls ADD COLUMN IF NOT EXISTS provider_call_id TEXT`,
`ALTER TABLE agent_tool_calls ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'native'`,
`ALTER TABLE agent_task_items ADD COLUMN IF NOT EXISTS semantic_key TEXT`,
`ALTER TABLE agent_task_items ADD COLUMN IF NOT EXISTS owner TEXT`,
`ALTER TABLE agent_task_items ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ`,
`CREATE UNIQUE INDEX IF NOT EXISTS agent_task_items_semantic_unique ON agent_task_items(task_id,semantic_key) WHERE semantic_key IS NOT NULL`,
`CREATE TABLE IF NOT EXISTS agent_scoped_memory (id BIGSERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, scope_type TEXT NOT NULL CHECK(scope_type IN ('user','server','project','conversation')), scope_id TEXT NOT NULL, key TEXT NOT NULL, value JSONB NOT NULL, source TEXT NOT NULL, confidence NUMERIC(4,3) NOT NULL DEFAULT 1, last_verified_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(user_id,scope_type,scope_id,key))`,
...
export async function installAgentRuntime(app, pool, options = {}) {
  ...
  app.get(`${prefix}/agent-runs`, auth, asyncRoute(async (req, res) => { const limit = page(req.query.limit, 50, 100), values = [req.agentUser.id], where = ["r.user_id=$1"]; if (req.query.status) { values.push(req.query.status); where.push(`r.status=$${values.length}`); } if (req.query.serverId) { values.push(Number(req.query.serverId)); where.push(`c.server_id=$${values.length}`); } if (req.query.search) { values.push(`%${req.query.search}%`); where.push(`(c.title ILIKE $${values.length} OR m.content ILIKE $${values.length} OR s.name ILIKE $${values.length} OR s.host ILIKE $${values.length})`); } values.push(limit); const rows = await pool.query(`SELECT r.public_id run_id,r.status,r.phase,r.started_at,r.completed_at,r.heartbeat_at,r.error,r.metadata,c.public_id conversation_id,c.title,c.server_id,s.name server_name,s.host,s.port,s.username,m.content prompt,COALESCE((SELECT COUNT(*) FROM agent_task_items i JOIN agent_tasks t ON t.id=i.task_id WHERE t.run_id=r.id AND i.status='completed'),0)::int completed_tasks,COALESCE((SELECT COUNT(*) FROM agent_task_items i JOIN agent_tasks t ON t.id=i.task_id WHERE t.run_id=r.id),0)::int total_tasks,COALESCE((SELECT SUM(charged_credits) FROM agent_usage_ledger u WHERE u.run_id=r.id AND u.status='settled'),0) credits_used FROM coding_agent_runs r JOIN conversations c ON c.id=r.conversation_id LEFT JOIN servers s ON s.id=c.server_id LEFT JOIN conversation_messages m ON m.id=(SELECT id FROM conversation_messages WHERE conversation_id=c.id AND role='user' AND run_id=r.id ORDER BY sequence LIMIT 1) WHERE ${where.join(" AND ")} ORDER BY r.created_at DESC LIMIT $${values.length}`, values); res.json({ items: rows.rows, limit }); }));
